...
## `params`
| 구분   | URL 예시    | 설명                          | Nuxt 접근 방법   |
| :----- | :---------- | :---------------------------- | :--------------- |
| Path   | /board/1    | URL 전체 경로 그 자체         | route.path       |
| Params | { id: '1' } | 경로 중 동적인 값만 추출한 것 | route.params.id  |
| Query  | ?page=2     | ? 뒤에 붙는 추가 옵션 데이터  | route.query.page |


- route.params.id
- `.id`가 잡히는 이유는 파일 이름이 `[id].vue`이기 때문
