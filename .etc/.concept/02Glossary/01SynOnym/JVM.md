...
- 표준 JVM: 코드를 실행할 때 실시간으로 해석하고 최적화(JIT)합니다. 이 때문에 프로그램을 켤 때 '예열(Warm-up)' 시간이 필요하고 메모리를 많이 사용합니다.
- 표준 JVM: C++로 작성된 'C2 컴파일러'를 사용합니다. 오랫동안 최적화되었지만, 코드가 너무 복잡해져서 새로운 최적화 기술을 도입하기가 매우 어렵습니다.